    ...
    try:
        with psycopg2.connect(os.environ["DATABASE_URL"]) as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT price FROM price_cache WHERE symbol=%s", (symbol,))
                row = cur.fetchone()
                return row[0] if row else None
    except Exception as e:
        logger.error(f"DB error in get_cached_price: {e}")
        return None
...
def get_all_user_ids():
    try:
        with psycopg2.connect(os.environ["DATABASE_URL"]) as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT DISTINCT user_id FROM alerts")
                return [row[0] for row in cur.fetchall()]
    except Exception as e:
        logger.error(f"DB error in get_all_user_ids: {e}")
        return []
